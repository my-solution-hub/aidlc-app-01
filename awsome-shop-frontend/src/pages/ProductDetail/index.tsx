...
import { getProduct, getRelatedProducts, listReviews, createReview } from "../../services/api/product";
import { listWishlist, addWishlist, removeWishlist } from "../../services/api/wishlist";
...
export default function ProductDetail() {
  ...
  const fetchSideData = useCallback(async () => {
    if (!id) return;
    const pid = Number(id);
    getRelatedProducts(pid).then(setRelated).catch(() => {});
    listReviews(pid).then(setReviews).catch(() => {});
    if (user) {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      listReviews(product.id).then(setReviews).catch(() => {});
    } catch (err) {
      snackbar.showError(err instanceof BusinessError ? err.message : t("employee.productDetail.reviewFailed"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3, p: "24px 32px" }}>
      <Button
        ...
        onClick={() => navigate(-1)}
        sx={{ alignSelf: "flex-start", textTransform: "none", color: "#64748B", fontSize: 14, fontWeight: 500, "&:hover": { bgcolor: "#F8FAFC" } }}
      >
        {t("employee.back")}
      </Button>
      ...
      {loading ? (
        ...
        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", py: 8, gap: 2 }}>
          ...
          <Typography sx={{ fontSize: 14, color: "#64748B" }}>{t("employee.noProducts")}</Typography>
        </Box>
      ) : (
        <>
          <Box sx={{ display: "flex", gap: "32px", bgcolor: "#fff", borderRadius: "12px", border: "1px solid #F1F5F9", p: "24px" }}>
            {/* Image gallery */}
            <Box sx={{ width: 360, flexShrink: 0, display: "flex", flexDirection: "column", gap: "12px" }}>
              <Box
                sx={{
                  width: 360,
                  height: 360,
            ...
            <Box sx={{ flex: 1, display: "flex", flexDirection: "column", gap: "16px" }}>
              <Typography sx={{ fontSize: 24, fontWeight: 700, color: "#1E293B" }}>{product.name}</Typography>

              <Box sx={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <Chip
                  label={product.category}
                  size="small"
                  sx={{ bgcolor: style.bg, color: style.color, fontWeight: 600, fontSize: 12, borderRadius: "8px" }}
                />
                {product.rating != null && product.rating > 0 && (
                  <Box sx={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <Rating value={product.rating} precision={0.1} size="small" readOnly />
                    <Typography sx={{ fontSize: 12, color: "#64748B" }}>
                      {product.rating.toFixed(1)} ({product.reviewCount ?? reviews.length})
                    </Typography>
                  </Box>
                )}
              ...
              <Box sx={{ display: "flex", alignItems: "flex-end", gap: "10px" }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  ...
                  <Typography sx={{ fontSize: 14, color: "#64748B", mb: "6px" }}>{t("employee.points")}</Typography>
                </Box>
                {product.marketPrice != null && product.marketPrice > 0 && (
                  <Typography sx={{ fontSize: 13, color: "#94A3B8", mb: "8px" }}>
                    {t("employee.productDetail.marketPrice")} ¥{product.marketPrice}
                  </Typography>
                )}
              </Box>

              {product.promotion && (
                <Box sx={{ alignSelf: "flex-start", bgcolor: "#FEF2F2", borderRadius: "6px", px: "10px", py: "4px" }}>
                  <Typography sx={{ fontSize: 12, fontWeight: 600, color: "#DC2626" }}>{product.promotion}</Typography>
                </Box>
              )}
              ...
              {product.description && (
                <Box sx={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  ...
                  <Typography sx={{ fontSize: 13, color: "#64748B", lineHeight: 1.7 }}>{product.description}</Typography>
                </Box>
              )}
              ...
              {product.specs && product.specs.length > 0 && (
                <Box sx={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  <Typography sx={{ fontSize: 14, fontWeight: 600, color: "#1E293B", mb: "4px" }}>
                    {t("employee.productDetail.specs")}
                  </Typography>
                  {product.specs.flatMap((row) =>
                    Object.entries(row).map(([k, v]) => (
                      <Box key={k} sx={{ display: "flex", gap: "16px", py: "6px", borderBottom: "1px solid #F8FAFC" }}>
                        <Typography sx={{ fontSize: 13, color: "#94A3B8", width: 120 }}>{k}</Typography>
              ...
              <Box sx={{ display: "flex", gap: "12px", mt: "auto" }}>
                <Button
                  ...
                  sx={{ borderRadius: "8px", px: "32px", py: "10px", fontSize: 15, fontWeight: 600, textTransform: "none" }}
                >
                  {product.status !== 1
                ...
                <Button
                  ...
                  startIcon={wished ? <FavoriteIcon sx={{ fontSize: 18 }} /> : <FavoriteBorderIcon sx={{ fontSize: 18 }} />}
                  sx={{
                    borderRadius: "8px",
                    px: "20px",
                    py: "10px",
                ...
                >
                  {wished ? t("employee.productDetail.wishlistRemove") : t("employee.productDetail.wishlistAdd")}
                </Button>
              </Box>
            </Box>
          </Box>
                ...
                {related.slice(0, 6).map((p) => {
                  ...
                  return (
                    <Box
                      ...
                      sx={{ cursor: "pointer", borderRadius: "8px", border: "1px solid #F1F5F9", overflow: "hidden", "&:hover": { boxShadow: 2 } }}
                    >
                      ...
                      <Box sx={{ p: "8px" }}>
                        <Typography sx={{ fontSize: 12, fontWeight: 600, color: "#1E293B", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {p.name}
                        </Typography>
                        <Typography sx={{ fontSize: 12, fontWeight: 700, color: "#D97706" }}>
          ...
          <Box sx={{ bgcolor: "#fff", borderRadius: "12px", border: "1px solid #F1F5F9", p: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {/* Submit review */}
            {user && (
              <Box sx={{ display: "flex", flexDirection: "column", gap: "8px", bgcolor: "#F8FAFC", borderRadius: "8px", p: "16px" }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <Typography sx={{ fontSize: 13, color: "#64748B" }}>{t("employee.productDetail.yourRating")}</Typography>
                  <Rating value={myRating} onChange={(_, v) => setMyRating(v)} size="small" />
                </Box>
                <TextField
                  fullWidth
                  size="small"
                  ...
                  sx={{ "& .MuiOutlinedInput-root": { borderRadius: "8px", fontSize: 14, bgcolor: "#fff", "& fieldset": { borderColor: "#E2E8F0" } } }}
                />
                <Button
                  variant="contained"
                ...
                >
                  {submitting ? <CircularProgress size={18} sx={{ color: "#fff" }} /> : t("employee.productDetail.submitReview")}
                </Button>
              </Box>
            )}
            ...
            {reviews.length === 0 ? (
              ...
              reviews.map((r) => (
                <Box key={r.id} sx={{ display: "flex", flexDirection: "column", gap: "4px", py: "12px", borderTop: "1px solid #F1F5F9" }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <Rating value={r.rating} size="small" readOnly />
                    <Typography sx={{ fontSize: 12, color: "#94A3B8" }}>{(r.createdAt ?? "").slice(0, 10)}</Typography>
                  </Box>
                  <Typography sx={{ fontSize: 13, color: "#1E293B" }}>{r.content}</Typography>
                </Box>
              ))
            )}
          </Box>
