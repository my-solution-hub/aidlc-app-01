...
- 格式：`/api/v1/{scope}/{module}/{action}`，版本号之后必须是三段
  ...
  - **第二段 `{module}`**：业务模块名（如 `order`、`category`、`brand`）
  - **第三段 `{action}`**：具体操作
- 示例：
  - `POST /api/v1/public/order/get` — 前端查询单个商品
  - `POST /api/v1/public/order/list` — 前端分页查询商品
  - `POST /api/v1/public/order/create` — 前端创建商品
  - `POST /api/v1/private/order/get` — 其他微服务内部查询商品
- 常用 action：
  - `/get` — 查询单条
  - `/list` — 分页查询
  - `/create` — 创建
...
- 返回值统一使用 `Result<T>` 包装（`com.awsome.shop.order.common.result.Result`）
