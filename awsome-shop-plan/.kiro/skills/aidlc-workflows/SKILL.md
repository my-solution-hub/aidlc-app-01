...
description: >
  AI-Driven Development Life Cycle (AI-DLC) — an adaptive software development workflow
  that guides structured development through three phases: Inception (WHAT/WHY), Construction (HOW),
  and Operations (DEPLOY/RUN). Use when: (1) User says "Using AI-DLC" or references AI-DLC methodology,
  (2) User wants structured, phase-driven software development with human oversight,
  (3) User needs adaptive workflow that assesses complexity and adjusts depth accordingly,
  (4) User wants requirements analysis, user stories, application design, code generation,
  and build/test planning in a structured workflow with explicit approvals at each stage.
---
...
**CRITICAL**: ALL output throughout the entire workflow MUST be in **Chinese (中文)**. This includes:
- All questions, responses, and messages displayed to the user
- All generated documentation and artifact files (requirements, user stories, design documents, plans, audit logs, etc.)
- All completion messages and approval prompts
- All content written to files in `aidlc-docs/`
- **NO EXCEPTIONS**: Even when referencing English technical terms, surrounding text MUST be in Chinese
...
## MANDATORY: File Reading Rule
**CRITICAL**: You MUST read files ONE AT A TIME, sequentially. NEVER attempt to read multiple files in a single operation or batch.
- Read one file → process its content → then read the next file
- **NO EXCEPTIONS**: Even when multiple files are listed together, read them one by one in order
...
**Common Rules**: ALWAYS load common rules at workflow start (read ONE file at a time, in order):
...
### Three-Level Checkbox Tracking System
- **Plan-Level**: Track detailed execution progress within each stage
- **Stage-Level**: Track overall workflow progress in aidlc-state.md
- **Execution-Plan-Level**: Track stage completion in `aidlc-docs/inception/plans/execution-plan.md`
- **Update immediately**: All progress updates in SAME interaction where work is completed
- **CRITICAL**: When marking a stage complete in `aidlc-state.md`, you MUST ALSO update the corresponding checkbox in `execution-plan.md` to keep both files in sync. NO EXCEPTIONS.
